% CODE TO BE RAN TO GET varMat MATRICES FOR:
% 1-2-3-4-5 tasks condition (prior, run m1_TrainingMatrices_Elektra) 
...
f_1task=1; %1 is only 1 task condition even when splitted in many tasks
...
for fileIndex = 1:length(Files)
    ...
    % Get condition info for each file   
   
    if f_1task==1
        fileDays{fileIndex,:} = Files{fileIndex,:}((unders(4)+1):(unders(5)-1));
    else
        fileDays{fileIndex,:} = Files{fileIndex,:}((unders(3)+1):(unders(4)-1));
    end
end

% Make the structure with as many tasks/conditions as there are
varMat = struct();
...
%% Fill out the info for each coondition/task
for fileIndex = 1:length(Files)
    ...
    for idxDay = 1:numel(varMat)
        ...
        if f_1task==1      
            Day = Files{fileIndex,:}((unders(4)+1):(unders(5)-1));
        else
    ...
    correctDayTask = idxDay;

    % Get trial Info
    %varMat(correctCondTask).Monkey = Files{fileIndex,:}((unders(2)+1):(unders(3)-1));
    if f_1task==1
        varMat(correctDayTask).Day = Files{fileIndex,:}((unders(4)+1):(unders(5)-1));
        
    else
        varMat(correctDayTask).Day = Files{fileIndex,:}((unders(3)+1):(unders(4)-1));
    end
    
    if ~isempty(strfind(Files{fileIndex,:},'PM'))      
        varMat(correctDayTask).PMdep = varTask(1);
        varMat(correctDayTask).PMind = varTask(2);
    elseif ~isempty(strfind(Files{fileIndex,:},'M1'))
        varMat(correctDayTask).M1dep = varTask(1);
        varMat(correctDayTask).M1ind = varTask(2);
    elseif ~isempty(strfind(Files{fileIndex,:},'S1'))        
        varMat(correctDayTask).S1dep = varTask(1);
        varMat(correctDayTask).S1ind = varTask(2);
    end
end
filesssep = strfind(FilesPath,'/');
disp(['right click and save varMat matrix of file:' FilesPath(filesssep(end-1):filesssep(end))])
...
% stdVAF = std(comp2)
% 
% % need to save the varMat matrices for final plot
