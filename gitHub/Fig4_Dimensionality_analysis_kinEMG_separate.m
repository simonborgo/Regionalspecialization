...
%     trialLength = size(DataAllGaits(1).EMG_cyclic,1);
...
%         
        ...
        for chan = 1:length(muscles)
            EMGfinal = [];
            for cond = 1:nCond
                EMGfinal = [EMGfinal, squeeze(EMG_avg(chan,cond,:))'];
            end
            EMGavg = [EMGavg; EMGfinal];
        end
% 
...
%             
            EMG_cond = EMG(:,cond,:,1:countCond(cond)-1);
            EMGavg_cond = squeeze(EMG_avg(:,cond,:));
            dimEMGall(cond) = m_DimensionalityEstimate_Simple(EMGavg_cond');
...
%         dimension = [dimPMall; dimM1all; dimS1all];
           dimension = [dimEMGall];
          Dim.EMG = m_DimensionalityEstimate_Simple(EMGavg');
...
%         Dim.S1 = m_DimensionalityEstimate_Simple(S1avg');
    ...
    saveDir = [dataDir 'DimEstimatesEMG' filesep];
    if ~isdir(saveDir)
        mkdir(saveDir)
    end
...
for Day = 1:size(sessions,1) % length(sessions)
    ...
    minSteps = min(countCond-1); % Minimum number of steps on one condition
        ...
        for chan = 1:length(goodKin)
            Kinfinal = [];
            for cond = 1:nCond
                Kinfinal = [Kinfinal, squeeze(Kin_avg(chan,cond,:))'];
            end
            Kinavg = [Kinavg; Kinfinal];
        end
...
%         
        ...
        for cond = 1:nCond
            ...
            Kin_cond = Kin(:,cond,:,1:countCond(cond)-1);
            Kinavg_cond = squeeze(Kin_avg(:,cond,:));
            dimKinall(cond) = m_DimensionalityEstimate_Simple(Kinavg_cond');
            

        end
          dimension = [dimKinall];
          Dim.Kin = m_DimensionalityEstimate_Simple(Kinavg');


    saveDir = [dataDir 'DimEstimatesKin' filesep];
    if ~isdir(saveDir)
        mkdir(saveDir)
    end
    save([saveDir Monkey '_' sessions(Day,:)],'dimension','Dim')
