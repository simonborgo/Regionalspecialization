...
for Day = 1:size(sessions,1)
    clearvars -except Day sessions dimPM dimM1 dimS1 dimPMall dimM1all dimS1all Monkey
    close all
...
%     nChan = nChan - length(IdxElectOut);
 ...
 %% Bad channel deletion (no channel removal because then each day has different number of dimensions)
%     if strcmp(Monkey,'Elektra')
%        
...
%     firingRatesAll = bsxfun(@minus,firingRatesAll,mean(firingRatesAll,3,'omitnan'));
    ...
    %     PE = firingRatesAll(neurons_PE,:,:,:);
    %     PE_avg = mean(firingRatesAll(neurons_PE,:,:,:),4,'omitnan');
    ...
    for cond = 1:nCond
        ...
        dimPM(:,cond,Day) = m_DimensionalityEstimate_SimpleFig(PMavg_cond');
        
        M1_cond = M1(:,cond,:,1:countCond(cond)-1);
        M1avg_cond = squeeze(M1_avg(:,cond,:));
        dimM1(:,cond,Day) = m_DimensionalityEstimate_SimpleFig(M1avg_cond');
        
        S1_cond = S1(:,cond,:,1:countCond(cond)-1);
        S1avg_cond = squeeze(S1_avg(:,cond,:));
        dimS1(:,cond,Day) = m_DimensionalityEstimate_SimpleFig(S1avg_cond');
        
    end
%     dimension = [dimPMall; dimM1all; dimS1all];
    
    dimPMall(:,Day) = m_DimensionalityEstimate_SimpleFig(PMavg');
    dimM1all(:,Day) = m_DimensionalityEstimate_SimpleFig(M1avg');
    dimS1all(:,Day) = m_DimensionalityEstimate_SimpleFig(S1avg');
...
%% Plot the mean across days for each cortex
% figure
% subplot(1,3,1)
% for i = 1:size(dimPM,2)
%     plot(mean(squeeze(dimPM(:,i,:)),2,'omitnan'),'color',color_Cyber(i,:))
%     hold on
% end
% plot(mean(dimPMall,2,'omitnan'),'k')
% plot([0 15],[90 90],'--','color',[0.5 0.5 0.5])
% xlim([1 15])
% ylim([20 100])
% xticks([1,15])
% yticks([20 100])
...
% 
% subplot(1,3,2)
% for i = 1:size(dimM1,2)
%     plot(mean(squeeze(dimM1(:,i,:)),2,'omitnan'),'color',color_Cyber(i,:))
%     hold on
% end
% plot(mean(dimM1all,2,'omitnan'),'k')
% plot([0 15],[90 90],'--','color',[0.5 0.5 0.5])
% xlim([1 15])
% ylim([20 100])
% xticks([1,15])
...
% 
% subplot(1,3,3)
% for i = 1:size(dimS1,2)
%     plot(mean(squeeze(dimS1(:,i,:)),2,'omitnan'),'color',color_Cyber(i,:))
%     hold on
% end
% plot(mean(dimS1all,2,'omitnan'),'k')
% plot([0 15],[90 90],'--','color',[0.5 0.5 0.5])
% xlim([1 15])
% ylim([20 100])
% xticks([1,15])
...
% set(gca,'TickDir','out')
% 
% set(gcf,'Position',[ 86         215        1080         270],'Renderer','painters')

%% Plot the shaded error bar mean across days for each cortex
figure
subplot(1,3,1)
shadedErrorBar(1:length(dimPMall),mean(dimPMall,2,'omitnan'),std(dimPMall,[],2)/sqrt(nDays),{'color',[0.5 0.5 0.5]});
hold on
for i = 1:size(dimPM,2)
    m = mean(squeeze(dimPM(:,i,:)),2,'omitnan');
    sd = std(squeeze(dimPM(:,i,:)),[],2)/sqrt(nDays);
    shadedErrorBar(1:length(m),m,sd,{'color',color_Cyber(i,:)});
end
plot([0 15],[90 90],'--','color',[0.5 0.5 0.5])
xlim([1 15])
ylim([20 100])
...
subplot(1,3,2)
shadedErrorBar(1:length(dimM1all),mean(dimM1all,2,'omitnan'),std(dimM1all,[],2)/sqrt(nDays),{'color',[0.5 0.5 0.5]});
hold on
for i = 1:size(dimM1,2)
    m = mean(squeeze(dimM1(:,i,:)),2,'omitnan');
    sd = std(squeeze(dimM1(:,i,:)),[],2)/sqrt(nDays);
    shadedErrorBar(1:length(m),m,sd,{'color',color_Cyber(i,:)});
end
plot([0 15],[90 90],'--','color',[0.5 0.5 0.5])
xlim([1 15])
...
subplot(1,3,3)
shadedErrorBar(1:length(dimS1all),mean(dimS1all,2,'omitnan'),std(dimS1all,[],2)/sqrt(nDays),{'color',[0.5 0.5 0.5]});
hold on
for i = 1:size(dimS1,2)
    m = mean(squeeze(dimS1(:,i,:)),2,'omitnan');
    sd = std(squeeze(dimS1(:,i,:)),[],2)/sqrt(nDays);
    shadedErrorBar(1:length(m),m,sd,{'color',color_Cyber(i,:)});
end
plot([0 15],[90 90],'--','color',[0.5 0.5 0.5])
xlim([1 15])
